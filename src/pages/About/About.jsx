import { aboutItems, aboutListItems, aboutStats, coreValues, aboutTeam } from "../../config/about.config";
...
const About = () => {
  return (
    <section className="p-8 lg:p-28">
        ...
        {aboutStats.map((item, index) => {
          return (
            <div className="grid justify-items-center p-8  m-8  border-harvestaLightGreen" key={index}>
              <span className="grid justify-items-center">{item.icon}</span> {/* REFACTOR THIS SECTION */}
      ...
      <div className="my-20">
        <h3 className="flex justify-center font-bold text-2xl mb-14">Our Core Values </h3>
       <div className="lg:grid grid-cols-3 gap-4">
      {coreValues.map((item, index)=>(
        <div className="bg-harvestaLightGreen rounded overflow-hidden shadow-md hover:shadow-lg text-white my-10" key={index}>
        <img src={item.img} alt="" className="w-full h-32 sm:h-48 object-cover"/>
        <div className="m-4">
          <span className="font-bold text-lg flex justify-center p-3 font-primary">{item.title}</span>
          <span className="block text-sm font-primary">{item.paragraph}</span>
        </div>
      </div>
      ))}
      ...
      <div className="p-4 font-primary">
        ...
        <h3 className=" uppercase text-gray-500 font-bold text-xs">Who are we?</h3>
        <h2 className="p-4 font-bold text-4xl text-harvestaLightGreen">Meet our team</h2>
        <p className="text-center text-gray-500 text-md text-wrap">Just take a look - each member of the team is watching your every gesture <br /> and will hear your every whisper.</p>
        </div>        
        <div className="lg:grid grid-cols-4 mt-10">
         {aboutTeam.map((item, index)=>(
          <div key={index} className="p-2 grid grid-cols-1 justify-items-center border-2 rounded-lg m-4 border-primary">
          <img src={item.img} alt="" className="w-40 h-40" />
          <h4 className="text-sm mt-4 font-semibold">{item.name}</h4>
          <h6 className="text-xs text-gray-500 uppercase my-2">{item.role}</h6>
          <p className="text-xs w-[220px] text-center text-gray-500 mb-1">{item.content}</p>
